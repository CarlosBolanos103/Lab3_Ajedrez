...
char *Piezas[] = {"PB", "PN", "CB", "CN", "AB", "AN", "TB", "TN", "DB", "DN", "RB", "RN"};
...
// mover(int jugador, char *movida, size_t longitud_movida)*/
...
int Mover_pieza(Tablero *pieza,int Jugador, char *movida ,size_t longitud_movida)
{
    ...
    // Se colocan alreves por la manera que se programaron los movimientos filasColumnas no ColumasFilas
...
int main()
{
    ...
    int opcion;
    int Jugador_seleccionado = Jugadores_turno();

    do
    {
        printf("Es el turno del Jugador %d \n", Jugador_seleccionado);
        printf("Elegi la opcion: 1-Mover pieza, 2-Reiniciar, 3-Salir:  ");
        scanf("%d", &opcion);
        ...
        {
        case 1:
            Pedir_Coordenadas(Cordenada);
            if (Mover_pieza(&Tablero,Jugador_seleccionado,Cordenada,4))
            {
               Jugador_seleccionado = Jugadores_turno();
            }
            Imprimir_tablero(&Tablero);
            printf("\n");
            break;
        ...
        default:
            printf("Eliga un opcion correcta\n");
            break;
        }
